		...
		return func(t *testing.T, s *mocks.MockCommitBatcherApi, pcb *sealing.CommitBatcher) promise {
			...
			go func() {
				...
				pcres, pcerr = pcb.AddCommit(ctx, si, sealing.AggregateInput{
					Info: minertypes.AggregateSealVerifyInfo{
						Number: sn,
					},
				})
			}()
...
func (f fakeProver) AggregateSealProofs(aggregateInfo minertypes.AggregateSealVerifyProofAndInfos, proofs [][]byte) ([]byte, error) {
